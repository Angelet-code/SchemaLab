...
| App web interactiva | En progreso | `AppWeb/index.html` | Branding definido como SchemaLab, subtitulo `Map the signal. Verify the build.`; conserva tarjetas laterales, centro sin tarjeta en vista de esquema, bloque de datos dinamico, zoom con rueda, pan con click de rueda, barra inferior con vistas y lista de componentes detallada filtrable por bloque desde la columna izquierda, incluyendo `0 Todo el circuito`; la lista se agrupa por tipo en tablas separadas, admite componentes compartidos entre bloques, enumera valvulas fisicas y muestra resistencias con simbolo `Ω` y codigo de colores. `AppWeb/code.html` queda como referencia Stitch original. |
